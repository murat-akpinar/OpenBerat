...
// The audit and explain screen (ADR-0026). It decides nothing: both endpoints
// check ADMIN_GROUP on the handler's first line, independent of the decision
// cache, so this file draws whatever they answered and a non-admin gets a page
// that says 403 rather than a page that hides itself.
//
...
// user outside ADMIN_GROUP and 500 when Postgres is unreachable; drawing the
// second as the first tells an operator their access was taken away, and
// drawing the first as the second sends them to check a database that is fine.
// --- Feature End ---
...
// `reset` fires before the fields are cleared, so the reload has to wait a turn
// or it re-reads the old values.
filterForm.addEventListener('reset', () => setTimeout(reload, 0));

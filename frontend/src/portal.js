...
json('/api/me')
  .then((me) => {
    ...
    // `admin` and `auditor` are the backend's answer, not a claim read out of
    // the token here — and they only decide whether a link is drawn. The
    // endpoints behind it authorise themselves (ADR-0026, ADR-0034).
    if (me.admin || me.auditor) document.getElementById('admin-link').hidden = false;
  })
  .catch(() => {});

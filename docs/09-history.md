# 09 — Build log: how phases 0–7 and the roadmap backlog closed

What `TODO.md` held while it was being done. Every box here is closed, and the
note under each one is what closing it actually took — the measurement, the
thing that turned out to be wrong, the ADR it forced. Kept because the numbers
in `docs/06` and the reasoning in the ADRs are stated there without their
workings, and this is the workings.

The standing backlog is in [`TODO.md`](../TODO.md). Decisions are in
[`adr/`](adr/).
...
- [x] **One real application, integrated end to end** — Jenkins behind the
      ...
      **200** with no login form in the body and `whoAmI` says `labuser`; with
      ...
      [ADR-0021](adr/0021-application-identity-trusted-headers.md) rather
...
- [x] **Admin screens, or a decision that there will not be any.**
      *There will not be any: **[ADR-0024](adr/0024-no-admin-ui-in-v1.md)**.
...
- [x] Backend on 2 instances + nginx health check (HA) — not a debt of this
      phase: N-06 puts it outside v1. It stayed in `TODO.md` until the roadmap
      backlog took it; it closed there, and the health check was deleted rather
      than built (below, last box).
...
- [x] **The audit + `explain` screen** — the one admin page ADR-0024 left to build
      *`/audit`, one page, no new endpoint and no state-changing call. Two ADRs
      ...
      ([ADR-0026](adr/0026-audit-explain-screen-in-v1.md)). And the screen
      **did not want the framework that was kept for it**: the frontend's
      ...
      loaded ([ADR-0027](adr/0027-frontend-no-framework.md)).
...
- [x] **The two questions that first box left** — answered, and both turned out
      to need code
      *[ADR-0029](adr/0029-path-pattern-is-a-subtree.md): a `path_pattern`
      ...
      exact-path form.**
      [ADR-0030](adr/0030-breakglass-generated-blocks.md): break-glass
...
- [x] **ADR: the decision cache with more than one instance** — the HA box's
      prerequisite
      *[ADR-0031](adr/0031-decision-cache-multi-instance.md): the cache
...
- [x] **The management plane is one password deep** — admin MFA, and the open
      ...
      *[ADR-0032](adr/0032-admin-mfa.md): **required for `ADMIN_GROUP`,
...
## Backlog — what reading an enterprise SSO roadmap found

Not a phase. This set came out of reading an enterprise SSO roadmap (`tmp/`, not
in Git) against the finished product, and it moved here as a unit once the last
box closed. Most of that document was somebody's deployment project — phases, a
responsibility matrix, Kubernetes, an API gateway — and none of it was product
work; what it *did* surface was a short list of places where this repo either
shipped one profile where four were wanted, or claimed something nothing
checked. **§ numbers below cite that document.** What was read and refused stays
in [`TODO.md`](../TODO.md), next to the list it guards, because a refusal nobody
...
- [x] **`/api/admin/*` is all-or-nothing, and the roadmap wants an auditor.**
      ...
      explain screens exist ([ADR-0026](adr/0026-audit-explain-screen-in-v1.md)),
      the write screens exist and are separately rate-limited
      ([ADR-0033](adr/0033-admin-write-screens.md)) — so the seam is
      ...
      *Needs an ADR before code: a second group means a second name in
      `.env.example`, and [ADR-0008](adr/0008-group-identity-name.md) owns
      how group names are matched. It also owes a requirement ID in `docs/06`.*
      **Closed: [ADR-0034](adr/0034-read-only-management-group.md), F-15.**
...
- [x] **Break-glass is written and timed at nothing.** `docs/08` and
      [ADR-0030](adr/0030-breakglass-generated-blocks.md) describe the way
      back in; §24 gives the equivalent criterion a number — a rollback to the
      old login **under 10–15 minutes**. [ADR-0017](adr/0017-fail-closed-availability.md)
      ...
      **1.1 s** off → on with the clock started at the `/readyz` probe `docs/08`
      ...
      ([ADR-0021](adr/0021-application-identity-trusted-headers.md) from the
...
- [x] **The audit partitions have never been restored.**
      [ADR-0022](adr/0022-audit-retention.md) drops a month as a partition
...
- [x] **Backend on 2 instances + nginx health check (HA — after the first
      ...
      **nginx OSS has no `health_check` directive** — only passive
      ...
      [ADR-0031](adr/0031-decision-cache-multi-instance.md) keeps the cache

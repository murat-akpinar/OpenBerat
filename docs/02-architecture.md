...
| `GET /api/admin/users` | admin, auditor | The realm's users, read **live from Keycloak** and stored nowhere ([ADR-0036](adr/0036-reset-second-factor.md)): `search` and `page` are Keycloak's own filter and paging. A row is the user's `sub` (Keycloak's own id, so the directory is where an operator gets one for a user who has never signed in), a username, an email, whether an OTP credential is enrolled (Keycloak's own `totp`, so a row costs no call of its own) and whether the user is in `ADMIN_GROUP` or `AUDITOR_GROUP` — the last from those two groups' member lists, two calls a page rather than one a user. This is not `known_user`: nothing is written, and the list is a view of the identity provider's directory rather than a directory of our own |
| `POST /api/admin/reset-second-factor` | admin | Deletes every `otp` credential the named user holds, so the next login is an enrolment (ADR-0036). Body `{"username": "<sAMAccountName>"}`. Three terminal refusals, all fail-closed: the caller's **own** account, a target in `ADMIN_GROUP` or `AUDITOR_GROUP` (which stays a Keycloak-console job under `KC_ADMIN_PASSWORD`, [ADR-0032](adr/0032-admin-mfa.md)), and a read the backend cannot make — a reset that cannot first prove the target is not privileged must not run. A name the realm does not have is a **404**, not an outage. Idempotent: a user with no credential answers 200 and `removed: 0` |

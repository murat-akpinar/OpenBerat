...
| N-06 | High availability (HA) | No in v1; the design will not prevent it — and that second half is now measured rather than asserted. Two backend instances run on `docker compose up --scale backend=2` with no configuration change: nginx reaches both through the per-request `resolver` and retries the survivor in the same request when one is stopped (30/30 served), and the kill switch reaches both in **0.11–0.14 s** over the [ADR-0031](adr/0031-decision-cache-multi-instance.md) broadcast against its 5 s target (`docs/07`). What keeps HA outside v1 is what has **not** been established: no instance count has been load-tested, and the first instance to add is nginx rather than the backend (`docs/07`) |
...
| Admin screens | One page with five tabs: the audit list, `explain`, who is signed in, application CRUD and entitlement mapping. Everything except revocation, which stays a deliberate terminal call | [0024](adr/0024-no-admin-ui-in-v1.md), [0026](adr/0026-audit-explain-screen-in-v1.md), [0033](adr/0033-admin-write-screens.md) |
...
- [ ] **Can upstream applications be reached bypassing nginx?** No longer three
      answers to choose between. [ADR-0021](adr/0021-application-identity-trusted-headers.md)
      makes (a) **network isolation** — upstreams reachable only from the PEP —
      a requirement of the product, because an application that learns the user
      from `X-Auth-*` is impersonable by anyone who can reach its port:
      measured, a forged header with no cookie authenticated as an admin from an
      unrelated host on the LAN (`docs/07`). What stays open is what to add on
      top for the case isolation cannot cover, where the same ADR's reversal
      trigger points: (b) mTLS, or (c) a **short-lived signed identity JWT** to
      the upstream plus a JWKS endpoint. (c) is the one that stands up to an
      audit, and it is a small piece of work.
